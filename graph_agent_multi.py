...
class DistGCDM(object):
    def __init__(self, g: DistGraph, args, evaluator, device, dev_id=None, **kwargs):
        ...
        # 训练
        self.model = DistGCN(g.ndata["features"].shape[1], args.hidden, self.n_class, nlayers=3,
...
class Condenser(object):
        ...
        while True:
            ...
            with model.join():
                ...
                for c in range(self.n_class):
                    ...
                    syn_g.ndata["features"] = self.feat_syn.data
    ...
    def loss_fn(self, dist_full, dist_syn):
        return compute_mmd(dist_full, dist_syn)
