...
if __name__ == "__main__":
    ...
    graph = dgl.distributed.DistGraph("ogb-product", part_config="data/ogb-product.json")
    labels = graph.ndata["labels"]
    nfeat = graph.ndata["features"]
    in_feats = nfeat.shape[1]
    n_classes = (labels.local_partition.max() + 1).item()
    # Create csr/coo/csc formats before launching sampling processes
    # This avoids creating certain formats in each data loader process, which saves momory and CPU.
    graph.local_partition.create_formats_()
    # Pack data
    data = (
        train_idx,
        val_idx,
